...
switch ($_POST["opcion"]){
	...
	case 'guardaryeditar':
		...
		if (empty($idusuario)){
         $rspta=$usuario->Insertar($idmacceso,$cod_usuario,$desc_usuario,$direccion,$telefono,$email,$clave,$imagen,$fechareg);
         echo $rspta=='1'?"Registro Ingresado Correctamente!":$rspta;        
		}
		else {
         $rspta=$usuario->Editar($idusuario,$idmacceso,$cod_usuario,$desc_usuario,$direccion,$telefono,$email,$clave,$imagen,$fechareg);
   ...
   break;
	...
	case 'eliminar':
        $rspta=$usuario->Eliminaracceso($idmacceso);
   ...
   break;
	...
	case 'listar':
		$rspta=$usuario->Listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
				...
				'<button class="btn btn-danger btn-xs" onclick="eliminar('.$reg->idusuario.',\''.$reg->idmacceso.'\')" data-toggle="tooltip" data-placement="right" title="Eliminar"><i class="fa fa-remove"></i></button></h5>'	 	 	
				:
				'<h5 small style="text-align:center; width:70px">
				<button class="btn btn-primary btn-xs" onclick="mostrar('.$reg->idusuario.')" data-toggle="tooltip" data-placement="right" title="Editar"><i class="fa fa-pencil-square-o"></i></button>'.
        ...
        "1"=>'<h5 style="width:100px">'.$reg->cod_usuario.'</h5>',
				...
				"3"=>'<h5 style="width:250px">'.$reg->cod_macceso.'-'.$reg->desc_macceso.'</h5>',                  
				"4"=>($reg->estatus)?
	...
	case 'verificar':
	    ...
	    //Declaramos las variables de sesión
			...
			$_SESSION['desc_macceso']=$fetch->desc_macceso;
	    $_SESSION['imagenu']=$fetch->imagen;
			...
			$_SESSION['estatus']=$fetch->estatus;
				...
				$marcados = $usuario->listarmarcados($fetch->idmacceso);
				...
				while ($per = $marcados->fetch_object())
				{
					...
					in_array(10,$valores)?$_SESSION['macceso']=1:$_SESSION['macceso']=0;
	...
	case 'selectMAcceso':
		...
		$macceso=new MAcceso();
		$rspta=$macceso->Select();

		while ($reg=$rspta->fetch_object()) 
		{
			 echo '<option value='.$reg->idmacceso.'>'.$reg->cod_macceso.'-'.$reg->desc_macceso.'</option>';
		}
 	break;

	case 'actclave':
		$rspta=$usuario->EditarClave($idusuario,$clave);
 		echo $rspta=='1'?"Registro Actualizado Correctamente!":$rspta;
	break;
