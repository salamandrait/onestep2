...
switch ($_POST['opcion']){
   ...
   case 'guardaryeditar':
      if (empty($idpago) && $_POST['tipopago']=='Banco'){
         ...
         $_POST['idcompra'],$_POST['montoh'],$_POST['idcuenta'],$_POST['idoperacion'],$_POST['cod_movbanco'],
         $_POST['tipoban'],$_POST['numerocb'],$_POST['fecharegb']);
         ...
         $_POST['idcaja'],$_POST['idoperacion'],$_POST['cod_movcaja'],$_POST['idcompra'],
         $_POST['montoh'],$_POST['fecharegc']);
         echo $rspta=='111'?"Registro Ingresado Correctamente!":$rspta;
         $rspta=$pago->ActSaldoCaja($_POST['idcaja']);
         $rspta=$pago->ActCod();
      }	else if (empty($idpago) && $_POST['tipopago']=='IVA'){

         $rspta=$pago->InsertarRetIva($idusuario,$idproveedor,$cod_pago,$desc_pago,$tipo,$origend,$origenc,$totalh,$fechareg,
         $_POST['idcaja'],$_POST['idoperacion'],$_POST['cod_movcaja'],$_POST['idcompra'],
         $_POST['montoh'],$_POST['fecharegc'],$_POST['retenciona']);
         echo $rspta=='111'?"Registro Ingresado Correctamente!":$rspta;
         $rspta=$pago->ActSaldoCaja($_POST['idcaja']);
         $rspta=$pago->ActCod();
      }
   ...
   case 'listar':
      ...
      while ($reg=$rspta->fetch_object()){
         ...
         if ($reg->estatus=='Registrado') {
            ...
            ' '.
            '<'.$btn.'primary" onclick="retiva" '.$dtg.'"Ret. I.V.A."> I.V.A.</button>'.
            '</h5>'
            :
            '<h5 small style="text-align:center; width:120px" class="small">
            <'.$btn.'primary" onclick="mostrar('.$reg->idpago.')" '.$dtg.'"Mostrar"><i class="fa fa-pencil-square-o"></i></button>'.
