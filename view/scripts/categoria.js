	...
	$.ajax({
	    ...
	    contentType: false,
		 ...
		 success: function(datos) {
			...
			if(datos==1062){
				bootbox.alert('El codigo <b>'+$("#cod_categoria").val()+'</b> ya se encuenta Registrado!. No es Posible Ingresar Registros Duplicados');
			}else{
				bootbox.alert(datos);
				mostrarform(false);
				listar();
...
{
	$.post("../ajax/categoria.php",{'opcion':'mostrar',idcategoria:idcategoria}, function(data)
	{
...
function eliminar(idcategoria)
{
	bootbox.confirm({message:"¿Está Seguro de Eliminar el Registro?",buttons:{
		...
		if(result)
        {
        	$.post("../ajax/categoria.php", {'opcion':'eliminar',idcategoria : idcategoria}, function(e){
...
function GenerarReporte(){		
	...
	$("#btnReporte").click(function(){
		$('#rep').prop({
			'href':'../reportes/rptCategoria.php',
			'target':'_blank',
		});
	});
};
