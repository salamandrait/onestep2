...
function init(){
	...
	$.post('../ajax/pais.php',{'opcion':'selectMoneda'},function (r) {
		if (r=='') {
	...
	tabla=$('#tblistado').dataTable(
	{
      ...
      columnDefs:[{
	    ...
	    buttons:[
			...
			exportOptions:{columns:[1,2,3,4,4]}},
			{
			extend:'excelHtml5',
	...
	$.ajax({
	    ...
	    contentType: false,
		 ...
		 success: function(datos) {
			...
			if(datos==1062){
				bootbox.alert('El codigo <b>'+$("#cod_pais").val()+'</b> ya se encuenta Registrado!. No es Posible Ingresar Registros Duplicados');
			}else{
				bootbox.alert(datos);
				mostrarform(false);		
				listar();
...
{
	$.post("../ajax/pais.php",{'opcion':'mostrar',idpais:idpais}, function(data)
	{
...
function eliminar(idpais)
{
	bootbox.confirm({message:"¿Está Seguro de Eliminar el Registro?",buttons:{
		...
		if(result)
        {
        	$.post("../ajax/pais.php", {'opcion':'eliminar',idpais : idpais}, function(e){
