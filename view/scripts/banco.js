...
function init(){
	...
	$.post('../ajax/banco.php',{'opcion':'selectMoneda'},function (r) {
		if (r=='') {
	...
	$.ajax({
	    ...
	    contentType: false,
		 ...
		 success: function(datos) {
			...
			if(datos==1062){
				bootbox.alert('El codigo <b>'+$("#cod_banco").val()+'</b> ya se encuenta Registrado!. No es Posible Ingresar Registros Duplicados');
			}else{
				bootbox.alert(datos);
				mostrarform(false);
				listar();
...
{
	$.post("../ajax/banco.php",{'opcion':'mostrar',idbanco:idbanco}, function(data)
	{
		...
		$("#plazo1").val(data.plazo1);
		$("#plazo2").val(data.plazo2);
...
function desactivar(idbanco)
{
	...
	callback:function(result){
		if(result)
			{
				$.post("../ajax/banco.php", {'opcion':'desactivar',idbanco : idbanco}, function(e){
					listar();
					bootbox.alert(e);
				});	
			}
...
function activar(idbanco)
{
	...
	callback:function(result){
		if(result)
			{
				$.post("../ajax/banco.php", {'opcion':'activar',idbanco : idbanco}, function(e){
					listar();
					bootbox.alert(e);
				});	
			}
...
function eliminar(idbanco)
{
	bootbox.confirm({message:"¿Está Seguro de Eliminar el Registro?",buttons:{
		...
		if(result)
        {
        	$.post("../ajax/banco.php", {'opcion':'eliminar',idbanco : idbanco}, function(e){
